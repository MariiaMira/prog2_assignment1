...
public class Exercise1Test {
    ...
    void bookCorrectToString(String title, String author, double price, boolean bound) {
        ...
        var parts = Set.of(title, author, String.format("%.1f", bound ? price * 1.3 : price), String.valueOf(bound));
    ...
    @CsvSource({"Phoebe Bridgers, Punisher, 2023, 10, 200.0, 200.0, 250.0", "Bruce Springsteen, The River, 1980, 10, 300.0, 300.0, 375.0"})
    void recordingCorrectToStringCD(String artist, String title, int year, int condition, double originalPrice, double price, double pricePlusVAT) {
        ...
        var strings = String.format("%s { name='%s', artist='%s', year=%d, condition=%d, original price=%.1f, price=%.1f, price+VAT=%.1f }%n", "CD", title, artist, year, condition, originalPrice, price, pricePlusVAT);
    ...
    void recordingCorrectToStringLP(String artist, String title, int year, int condition, double originalPrice, double price, double pricePlusVAT) {
        ...
        var strings = String.format("%s { name='%s', artist='%s', year=%d, condition=%d, original price=%.1f, price=%.1f, price+VAT=%.1f }%n", "LP", title, artist, year, condition, originalPrice, price, pricePlusVAT);
...
LP { name='Giant Steps', artist='John Coltrane', year=1959, condition=10, original price=100,0, price=425,0, price+VAT=531,3 }
...
*/
        ...
        part = Set.of("Giant Steps", "John Coltrane", "1959", "LP", "10", "100.0", "425.0", "531.3");
        assertTrue(part.stream().allMatch(receipt::contains), String.format(message, part, receipt));
...
LP { name='Little Oblivions', artist='Julien Baker', year=2021, condition=10, original price=120,0, price=135,0, price+VAT=168,8 }
 ...
 */
        ...
        part = Set.of("Little Oblivions", "Julien Baker", "2021", "LP", "10", "120.0", "168.8");
        assertTrue(part.stream().allMatch(receipt::contains), String.format(message, part, receipt));
    }
}